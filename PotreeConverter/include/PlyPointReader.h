...
//std::cout << "size " << B.size() <<std::endl;														            
		...
		if(format == PLY_FILE_FORMAT_ASCII){
			...
			for(const auto &prop : vertexElement.properties){
				...
				if(prop.name == "x" && prop.type.name == plyPropertyTypes["float"].name){
					...
					intensity = stof(token);

					intensity = (unsigned short)(intensity_scale * (stof(token) - intensity_min));
					//std::cout << (unsigned short)((65000/0.012) * (stof(token)-0.01)+1) <<std::endl;
				}
		...
		int color_lookup = intensity *255/65535;
		r = 255 * R[color_lookup];
		g = 255 * G[color_lookup];
		b = 255 * B[color_lookup];
		...
		//std::cout << "intensity set to " << intensity <<std::endl;
		//add end
		pointsRead++;
		return true;
	}
